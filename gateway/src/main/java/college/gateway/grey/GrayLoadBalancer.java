...
public class GrayLoadBalancer implements ReactorServiceInstanceLoadBalancer {
    ...
    private List<ServiceInstance> filterTagServiceInstances(List<ServiceInstance> instances, HttpHeaders headers) {
        // 情况一，没有 tag 时，直接返回
        String tag = getTag(headers);
        if (StrUtil.isEmpty(tag)) {
            return instances;
        }

        // 情况二，有 tag 时，使用 tag 匹配服务实例
        List<ServiceInstance> chooseInstances = instances.stream().filter(instance -> tag.equals(getTag(instance))).collect(Collectors.toList());
        if (CollUtil.isEmpty(chooseInstances)) {
    ...
    public static String getTag(HttpHeaders headers) {
        ...
        return Objects.equals(tag, HOST_NAME_VALUE) ? StrUtil.blankToDefault(NetUtil.getLocalHostName(), IdUtil.fastSimpleUUID()) : tag;
    }
